...
async function router(ctx: Context): Promise<Handler> {
  ...
  if (user?.api_u_id == "-1" || !user || user?.reloadFromApi == true) {
    console.log("REQUESTING USER API_ID REQ:", {
    ...
    if (userData.data.status === "error") {
      ...
      ctx.details.lang = {
        ...
        native:
          API_CONSTANTS.data.data.langs[ctx.constants.data.default_lang].native,
      };
    ...
    await userList.push(ctx.userID.split("@")[0], {
      ...
      settings: {
        lang: {
          iso: API_CONSTANTS.data.data.langs[
            userData.data.data.user[Object.keys(userData.data.data.user)[0]]
...
client.on("message", async (msg) => {
  ...
  if (!["79138306352@c.us"].includes(userId)) {
    return;
  }
  if (userId === "79999183175@c.us" || userId === "79029403313@c.us") {
    return;
  }
  if (userId === "status@broadcast") {
    return;
  }
  userId = "24642624642624@c.us";
  ...
  const ctx: Context = {
    ...
    user: {
      settings: {
        lang: {
          iso: API_CONSTANTS.data.data.langs[API_CONSTANTS.data.default_lang]
            .iso,
          api_id: API_CONSTANTS.data.default_lang,
          native:
            API_CONSTANTS.data.data.langs[API_CONSTANTS.data.default_lang]
              .native,
        },
      },
    },
