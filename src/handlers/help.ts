...
export async function HelpHandler(ctx: Context): Promise<void> {
    ...
    if(state?.id === 'order' && state?.state === 'collectionFrom'){
        await ctx.chat.sendMessage(ctx.constants.getPrompt(localizationNames.helpStartPoint, ctx.user.settings.lang.api_id ));
        return
    } else if(state?.id === 'order' && state?.state === 'collectionTo'){
        await ctx.chat.sendMessage(ctx.constants.getPrompt(localizationNames.helpEndPoint, ctx.user.settings.lang.api_id ));
        return
    }
